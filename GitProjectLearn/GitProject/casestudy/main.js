...
function displayAllProduct() {
    ...
    for (let i = start; i < quantity; i++) {
        if (arrProduct[i] == undefined) {
            ...
            data += "<tr>";
            data += "<td class='product'><a target='_blank' href='" + arrProduct[i].src + "'><img style='width:120px;height:100px;' src='"+ arrProduct[i].image + "' alt='Lỗi'></a></td>";
    ...
    data += "</table>";
    document.getElementById("productAll").innerHTML = data;
}

function nextPage() {
    if (start < arrProduct.length - 3) {
        start += 3
        quantity += 3
        displayAllProduct()
    }
