...
function displayAllProduct() {
    ...
    for (let i = start; i < quantity; i++) {
        if (arrThoiTrang[i] == undefined) {
            ...
            data += "<tr>";
            data += "<td class='product'><a target='_blank' href='" + arrThoiTrang[i].src + "'><img style='width:120px;height:100px;' src='"+ arrThoiTrang[i].image + "' alt='Lỗi'></a></td>";
            data += "<td class='product'>" + arrThoiTrang[i].name + "</td>";
            data += "<td class='product'>" + arrThoiTrang[i].price + "</td>";
            data += "<td class='product'>" + arrThoiTrang[i].status + "</td>";
            data += "</tr>";
        }
    }
    data += "</table>";
    document.getElementById("productThoiTrang").innerHTML = data;
}

function nextPage() {
    if (start < arrThoiTrang.length - 3) {
        start += 3
        quantity += 3
        displayAllProduct()
    }
