...
function displayAllProduct() {
    ...
    for (let i = start; i < quantity; i++) {
        if (arrTienIch[i] == undefined) {
            ...
            data += "<tr>";
            data += "<td class='product'><a target='_blank' href='" + arrTienIch[i].src + "'><img style='width:120px;height:100px;' src='"+ arrTienIch[i].image + "' alt='Lỗi'></a></td>";
            data += "<td class='product'>" + arrTienIch[i].name + "</td>";
            data += "<td class='product'>" + arrTienIch[i].price + "</td>";
            data += "<td class='product'>" + arrTienIch[i].status + "</td>";
            data += "</tr>";
        }
    }
    data += "</table>";
    document.getElementById("productTienIch").innerHTML = data;
}

function nextPage() {
    if (start < arrTienIch.length - 3) {
        start += 3
        quantity += 3
        displayAllProduct()
    }
